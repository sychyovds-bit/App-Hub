...
export function init(container) {
  container.innerHTML = `
    <h1>Цвета CSS</h1>
    <p class="subtitle">Клик по цвету копирует HEX-код</p>
    <div class="colors-search-box">
      <input id="colorsSearch" type="text" placeholder="Поиск цвета..." aria-label="Поиск цвета">
      <span class="colors-count" id="colorsCount"></span>
    </div>
    <div class="colors-grid" id="colorsGrid"></div>
  `;

  const grid = container.querySelector('#colorsGrid');
  const search = container.querySelector('#colorsSearch');
  const count = container.querySelector('#colorsCount');
  ...
  function render(query = '') {
    ...
    for (const [name, hex] of Object.entries(CSS_COLORS)) {
      if (q && !name.includes(q) && !hex.includes(q)) continue;
      shown++;
      const cell = document.createElement('button');
      cell.className = 'color-cell';
      cell.title = `${name} — копировать`;
      cell.setAttribute('aria-label', `Скопировать ${hex}`);
      cell.innerHTML = `
        <span class="color-cell-swatch" style="background:${hex}"></span>
        <span class="color-cell-name">${name}</span>
        <span class="color-cell-hex">${hex}</span>
      `;
      cell.addEventListener('click', () => {
        navigator.clipboard.writeText(hex).then(() => toast(`${name}: ${hex} скопирован`, 'success'));
      });
      grid.appendChild(cell);
    }
    count.textContent = `${shown} из ${Object.keys(CSS_COLORS).length}`;
  }
